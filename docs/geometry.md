...
```JavaScript
<Geometry position={[-2.5, 0, -0.1]} scale={[1.0, 1.0, 1.0]}
...
An array of strings that each represent a material that was created via Materials.createMaterials(). Material i is applied to the submesh defined by triangleIndices i . If there are fewer materials than submeshes, then submesh i will use Material i % numMaterials.
