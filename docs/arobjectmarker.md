...
ARObjectMarker is a component that attempts to find the given target in the user's view allowing the developer to place objects with respect to the object or to determine their location relative to the object target.

This component has the exact same signature as ARImageMarker but its target uses the new "Object" target type.
...
<ARObjectMarker target={"targetOne"} >
...
Name of the object target created with ARTrackingTargets.
...
Called when an market has been clicked.
