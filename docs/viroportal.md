...
A component that represents the entry way into a ViroPortalScene. A Viro3DObject must be provided with an opacity around [0,0,0] in model coordinates in order for the portal to behave properly.
...
True if events should use the geometry of the object to determine if the user is interacting with this object. If false, the object's axis-aligned bounding box will be used instead. Enabling this is more accurate but takes more processing power, so it is set to false by default.s
