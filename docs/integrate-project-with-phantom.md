...
The instructions below explain how to integrate PhantomReact with your existing iOS project. We use the following github project as an example: https://github.com/JoelMarcey/iOS-2048.

We'll take this app and modify it by adding a PhantomReact VR or AR view to the application. This doc will show you how to:

1. Add React Native and PhantomReact to your existing iOS application.
2. Launch PhantomReact from your existing iOS Application.
3. Pass parameters from your native app to Phantom.

## Prerequisites
Complete the first 7 steps of the Set up Xcode with Phantom guide to install dependencies.
...
If you are already using CocoaPods, simply add PhantomReact to your Podfile.
...
var Sample = createReactClass({
  render: function() {
    ...
    // the initial properties from this base component to the SceneNavigator
    ...
    return (
      <SceneNavigator
        ...
        AppProps={AppProps}
        vrModeEnabled={AppProps.vrModeEnabled}
...
AppRegistry.registerComponent('RNHighScores', () => PhantomSample);
```
This file is the main entry point into the iOS version of your application. Essentially, it creates a basic React class (PhantomSampleApp) which wraps a SceneNavigator and sets the initial scene to a TestScene module defined at ./js/TestScene which we'll create in the next step. It then registers RNHighScores with the AppRegistry so that we can launch it by name from objective-C.
...
#endif
```
Now that we finished modifying the React Native codebase, it's time to modify your iOS code to enter a PhantomReact Scene. The easiest way to do this is to add an event (button click, etc) to an existing view controller. Then in the event callback, create and launch a UIViewController with a RCTRootView containing the PhantomReact app specified in the above index.js file.

Let's do this with the example we are using.

First let's edit the apps AppDelegate, in this case F3HAppDelegate. Open F3HAppDelegate.h and add the property isSceneDisplaying:
...
@property (nonatomic, assign) BOOL isSceneDisplaying;
...
- (UIInterfaceOrientationMask)application:(UIApplication *)application supportedInterfaceOrientationsForWindow:(UIWindow *)window {
  // Force the oprientation into landscape mode if we are displaying a SceneViewController.
  if (self.isSceneDisplaying) {
    return UIInterfaceOrientationMaskLandscapeRight;
  }
  return UIInterfaceOrientationMaskAllButUpsideDown;
}
```
This ensures that when a PhantomReact scene displays the rotation will be set to landscape at the app level.

Now lets create a UIViewController derived class called ViewController. The header file should look like below:
...
@end
```
Now in ViewController.m add the following methods so the file looks like below:
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    delegate.isSceneDisplaying = YES;
  
  // register for the exit notification
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(exitPhantomReact)
                                               name:kVRTOnExitPhantom
                                             object:nil];
...
- (void)exitPhantomReact {
  dispatch_async(dispatch_get_main_queue(), ^{
    F3HAppDelegate *delegate = (F3HAppDelegate *)[[UIApplication sharedApplication] delegate];
    delegate.isSceneDisplaying = NO;
    if (self.vrMode) {
...
1. Tell our appDelegate we are displaying our VR scene by setting isSceneDisplaying to true.

2. Register to listen for notification kVRTOnExitPhantom. This is a notification that PhantomReact invokes to let you know that PhantomReact is exiting so you can perform your own cleanup. In this case we state we want to invoke the exitReactPhantom method.

The exitReactPhantom method is invoked when we exit the PhantomReact View. Depending on if vrMode is true or false, we signal to our parent controller that we are dismissing. In addition, we tell the appDelegate we are no longer displaying a PhantomReact scene and remove ourselves as a listener for NSNotificationCenter.

Now let's use our new ViewController and integrate it into our current app.
...
- (IBAction)highScoreButtonPressed:(id)sender {
  ...
  [[RCTRootView alloc] initWithBundleURL : jsCodeLocation
                       ...
                       initialProperties :initialProperties
                        launchOptions    : nil];
  ViewController *vc = [[ViewController alloc] initWithVREnabledMode:vrEnabled];
  vc.view = rootView;
...
## Adding a button to launch Phantom
...
And Voilà! you've now added a PhantomReact Scene into your application.
...
## Run the App
Now you are almost ready to run your PhantomReact integrated app:
...
You now have added PhantomReact to an existing app!
...
## Troubleshooting
Please refer to the troubleshooting section at the bottom of the Installing Phantom guide.