...
A composite component that provides an animated spinner that can be used for providing feedback to the user while assets are loading.
...
Cartesian position of the spinner in 3D world space, specified as [x, y, z].

**rotation**	PropTypes.arrayOf(PropTypes.number)

The rotation of the spinner around it's local axis specified as Euler angles [x, y, z]. Units for each angle are specified in degrees.
...
The scale of the spinner in 3D space, specified as [x,y,z]. A scale of 1 represents the current size of the spinner. A scale value of < 1 will make the spinner proportionally smaller while a value >1 will make the spinner proportionally bigger along the specified axis.
...
Spinner visual type for either a light or dark theme. This defaults to dark.
...
False if the spinner should be hidden. By default the spinner is visible and this value is true.
