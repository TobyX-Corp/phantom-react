 ...
 *
 * @providesModule ARSceneNavigator
 * @flow
 */
...
var ARSceneNavigatorModule = require('react-native').NativeModules.VRTARSceneNavigatorModule;
...
/**
 * ARSceneNavigator is used to transition between multiple AR Scenes.
 */
var ARSceneNavigator = createReactClass({
  propTypes: {

      ...View.propTypes,

      /**
       * ARSceneNavigator uses "scene" objects like the following to
       * describe a scene.
       */
  ...
  popN: function(n: number) {
    ...
    if (this.state.sceneHistory.length - n <= 0){
        console.log("WARN: Attempted to pop the root scene in ARSceneNavigator!")
        return;
    }
  ...
  _startVideoRecording(fileName, saveToCameraRoll, onError) {
    ARSceneNavigatorModule.startVideoRecording(findNodeHandle(this), fileName, saveToCameraRoll, onError);
  },
  ...
  async _stopVideoRecording() {
    return await ARSceneNavigatorModule.stopVideoRecording(findNodeHandle(this));
  },
  ...
  async _takeScreenshot(fileName, saveToCameraRoll) {
    return await ARSceneNavigatorModule.takeScreenshot(findNodeHandle(this), fileName, saveToCameraRoll);
  },


  async _project(point) {
    return await ARSceneNavigatorModule.project(findNodeHandle(this), point);
  },

  async _unproject(point) {
    return await ARSceneNavigatorModule.unproject(findNodeHandle(this), point);
  },
  ...
  _resetARSession(resetTracking, removeAnchors) {
    ARSceneNavigatorModule.resetARSession(findNodeHandle(this), resetTracking, removeAnchors);
  },
  ...
  _setWorldOrigin(worldOrigin) {
    ARSceneNavigatorModule.setWorldOrigin(findNodeHandle(this), worldOrigin)
  },
  ...
  render: function() {
    ...
    //checkMisnamedProps("ARSceneNavigator", this.props);
...
var VRTARSceneNavigator = requireNativeComponent(
    'VRTARSceneNavigator', ARSceneNavigator, {
        nativeOnly: { currentSceneIndex:true }
    }
);

module.exports = ARSceneNavigator;
