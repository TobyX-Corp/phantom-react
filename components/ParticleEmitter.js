 ...
 *
 * @providesModule ParticleEmitter
 * @flow
 */
'use strict';
...
var ParticleEmitter = createReactClass({
  ...
  render: function() {
    ...
    checkMisnamedProps("ParticleEmitter", this.props);
    ...
    if (this.props.particleAppearance && this.props.particleAppearance.color){
      ...
      if (colorModifier.initialRange.length != 2){
        console.error('The <ParticleEmitter> component requires initial value of [min, max] when defining inital rotation property!');
        return;
      }
