...
cp -r ./node_modules/phantom-react/bin/files/javascript/* .

# replace APP_NAME_HERE with actual app names
vsed "s/APP_NAME_HERE/$PHANTOM_PROJECT_NAME/" index.*
...
LINE_TO_ADD="  \"prestart\": \"./node_modules/phantom-react/bin/run_ngrok.sh\","
LINE_TO_ADD=$(echo $LINE_TO_ADD | sed -e 's/[]\/$*.^|[]/\\&/g')

vsed "s/$LINE_TO_APPEND_AFTER/&"$'\\\n'"$LINE_TO_ADD/" $TARGET_FILEPATH

chmod +x ./node_modules/phantom-react/bin/run_ngrok.sh
